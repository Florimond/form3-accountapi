...
Author of this solution: **Florimond Husquinet**.

### As requested

- The docker compose file was modified so running `docker-compose up` will start the server and run the tests.
- The solution aims at being as generic as possible. While it provides the necessary datastructures to deal with Accounts, it can easily be used for any other type of resources in the Form3 API and even other APIs.

### Extras

- The solution implements the `List` operation, as I started the exercise before this was no longer required.
- The solution implements automatic retries with an exponential backoff mechanism, and requests can be cancelled though the Context. This could be categorized as an "advanced feature", however it seemed to be more than just a nice-to-have to me.
